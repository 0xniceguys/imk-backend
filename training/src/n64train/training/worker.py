...
def run_worker(
    ...
    while valid_episodes < episodes_per_worker and attempt_idx < max_attempts:
        ...
        try:
            ...
            # With verified health addresses (u8 read) and health_ready poll
            ...
            while time.time() - ep_start < MAX_EPISODE_SECS:
                ...
                if ep_steps % 50 == 1 and next_state is not None:
                    ...
                    # Optional deep scan for reverse-engineering candidate addresses.
                    # Disabled by default because it is noisy and can be mistaken for
                    # the actual traced health used by training.
                    if os.getenv('MK4_TRACE_SCAN') == '1':
                        try:
                            _cands = [
                                ('OLD_P1', 0x8036E729), ('OLD_P2', 0x8036E72E),
                                ('0E_DC6', 0x800E0DC6), ('0E_E26', 0x800E0E26),
                                ('0E_E0E', 0x800E0E0E), ('0E_B0E', 0x800E0B0E),
                                ('CDB97',  0x800CDB97), ('F10C5',  0x800F10C5),
                                ('36_F245', 0x8036F245), ('93A23',  0x80093A23),
                            ]
                            _vals = ' '.join(f'{n}={tracer.helper.read_u8(a)}' for n, a in _cands)
                            print(f'[worker-{worker_id}] SCAN step={ep_steps} {_vals}', flush=True)
                        except Exception as _e:
                            print(f'[worker-{worker_id}] SCAN step={ep_steps} ERROR: {_e}', flush=True)
